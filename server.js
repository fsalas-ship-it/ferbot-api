...
// 🔵 NUEVO: middleware de explicación (no rompe nada)
const explainMiddleware = require("./explain-middleware");

const app = express();
app.use(cors());
app.use(express.json({ limit: "1mb" }));

// 🔵 NUEVO: activar middleware (antes de las rutas)
app.use(explainMiddleware());
...
// ----------------------------
// ASSIST con OpenAI (GPT-5 Thinking, si pones tu API Key)
// ----------------------------
app.post("/assist_openai", async (req, res) => {
  try {
    ...
    const system = [
      ...
      "No envíes precios ni agendes en etapas de integración, sondeo o rebatir.",
    ...
    const r = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model,
        temperature: 1, // ✔️ compat con modelos que sólo soportan 1
        messages: [
